...
@Component({
  ...
  styleUrls: ['./complain-type.component.css']
})
export class ComplainTypeComponent implements OnInit {
  ...
  complaintType = '';
  seletedSetupId = null;
  componentLabel = 'complain type';

  submitForm(): void {
    if (this.complaintType == null || this.complaintType === '') {
      this.error = `Please enter ${this.componentLabel} name`;
    } else {
      this.error = '';
      this.isCreatingSetup.next(true);
      this.setup
        .genericPost('setups/complainttypes', { name: this.complaintType, status: 'ACTIVE' })
        .pipe(first())
        .subscribe(
          success => {
  ...
  toggleItem($event: any, item: any) {
    this.setup.toggleActive(`setups/complainttypes/${item.id}`, $event ? 'ACTIVE' : 'INACTIVE').pipe(first())
      .subscribe(toggled => {
  ...
  update() {
    if (!this.updateForm.valid) {
      ...
      }, `setups/complainttypes/${this.seletedSetupId}`).pipe(first()).subscribe(
        response => {
  ...
  deleteSetup(item: any) {
    this.setup.deleteSetup(`setups/complainttypes/${item.id}`).pipe(first()).subscribe(
      res => {
        this.getList();
