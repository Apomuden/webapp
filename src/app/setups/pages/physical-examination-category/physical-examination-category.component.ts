...
@Component({
  selector: 'app-physical-examination-category',
  templateUrl: './physical-examination-category.component.html',
  styleUrls: ['./physical-examination-category.component.css']
})
export class PhysicalExaminationCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getPhysicalExaminationCategories();
  }


  getPhysicalExaminationCategories() {
    this.isLoadingCategories = true;
    this.setup.getHistoryCategories('setups/physicalexaminationcategories')
      .pipe(first())
      .subscribe(
        res => {
  ...
  createCategory() {
    if (this.createCategoryForm.valid) {
      ...
      this.setup.createHistoryCategory('setups/physicalexaminationcategories',
  ...
  deleteCategory(category) {
    if (category) {
      this.setup.deleteSetup(`setups/physicalexaminationcategories/${category.id}`).pipe(first()).subscribe(
        res => {
          if (res) {
  ...
  update() {
    if (this.updateForm.valid) {
      ...
      this.setup.updateSetup(this.updateForm.value, `setups/physicalexaminationcategories/${this.categoryId}`)
        .pipe(first()).subscribe(
          res => {
