...
export class AgeGroupSetupComponent
  ...
  ngAfterViewInit() {
    this.ageGroupForm.valueChanges
    ...
    if (!this.updateForm.valid) {
      ...
      this.setup
        .updateSetup(
          ...
          `setups/agegroups/${this.ageGroupId}`
        )
        .pipe(first())
        .subscribe(
          response => {
  ...
  deleteAgeGroup(agegroup: any) {
    this.setup
      .deleteSetup(`setups/agegroups/${agegroup.id}`)
      .pipe(first())
      .subscribe(
        res => {
          this.getAgeGroups();
  ...
  toggleItem($event: any, ageGroup: any) {
    this.setup
      .toggleActive(
        `setups/agegroups/${ageGroup.id}`,
        $event ? "ACTIVE" : "INACTIVE"
      )
      .pipe(first())
      .subscribe(
        toggled => {
          const index = this.ageGroups.findIndex(
            group => group.id === toggled.id
          );
          this.ageGroups[index].isActivated = toggled.isActivated;
        },
        error => {
          const index = this.ageGroups.findIndex(
            group => group.id === ageGroup.id
          );
          this.ageGroups[index].isActivated = !ageGroup.isActivated;
          this.notification.error(
            "Toggle failed",
            "Unable to toggle this item."
          );
        }
      );
  }
