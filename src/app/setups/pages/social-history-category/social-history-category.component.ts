...
@Component({
  selector: 'app-social-history-category',
  templateUrl: './social-history-category.component.html',
  styleUrls: ['./social-history-category.component.css']
})
export class SocialHistoryCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getSocialHistoryCategories();
  }


  getSocialHistoryCategories() {
    this.isLoadingCategories = true;
    this.setup.getHistoryCategories('setups/socialhistorycategories')
      .pipe(first())
      .subscribe(
        res => {
  ...
  getHistoryItems(selectedCategory: any) {
    ...
    this.setup.genericGet('setups/socialhistories').pipe(first()).subscribe(
      res => {
        this.isLoadingItems = false;
        this.categoryList = res.filter(item => item.social_history_category_id === selectedCategory.id);
      },
      err => {
        this.isLoadingItems = false;
      }
  ...
  createCategory() {
    if (this.createCategoryForm.valid) {
      ...
      this.setup.createHistoryCategory('setups/socialhistorycategories',
  ...
  createItem() {
    if (this.createItemForm.valid) {
      ...
      this.setup.genericPost('setups/socialhistories',
        {
          ...this.createItemForm.value, status: 'ACTIVE',
          social_history_category_id: this.selectedCategory.id
  ...
  deleteCategory(category) {
    if (category) {
      this.setup.deleteSetup(`setups/socialhistorycategories/${category.id}`).pipe(first()).subscribe(
        res => {
          if (res) {
  ...
  deleteItem(item) {
    if (item) {
      this.setup.deleteSetup(`setups/socialhistories/${item.id}`).pipe(first()).subscribe(
        res => {
          if (res) {
  ...
  update() {
    if (this.updateForm.valid) {
      ...
      this.setup.updateSetup(this.updateForm.value, `setups/socialhistorycategories/${this.categoryId}`)
        .pipe(first()).subscribe(
          res => {
