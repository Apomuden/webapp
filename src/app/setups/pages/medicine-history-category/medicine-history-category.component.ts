...
@Component({
  selector: 'app-medicine-history-category',
  templateUrl: './medicine-history-category.component.html',
  styleUrls: ['./medicine-history-category.component.css']
})
export class MedicineHistoryCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getMedicineHistoryCategories();
  }


  getMedicineHistoryCategories() {
    this.isLoadingCategories = true;
    this.setup.getHistoryCategories('setups/medicinehistorycategories')
      .pipe(first())
      .subscribe(
        res => {
  ...
  getHistoryItems(selectedCategory: any) {
    ...
    this.setup.genericGet('setups/medicinehistories').pipe(first()).subscribe(
      res => {
        this.isLoadingItems = false;
        this.categoryList = res.filter(item => item.medicine_history_category_id === selectedCategory.id);
      },
      err => {
        this.isLoadingItems = false;
      }
  ...
  createCategory() {
    if (this.createCategoryForm.valid) {
      ...
      this.setup.createHistoryCategory('setups/medicinehistorycategories',
  ...
  createItem() {
    if (this.createItemForm.valid) {
      ...
      this.setup.genericPost('setups/medicinehistories',
        {
          ...this.createItemForm.value, status: 'ACTIVE',
          medicine_history_category_id: this.selectedCategory.id
  ...
  deleteCategory(category) {
    if (category) {
      this.setup.deleteSetup(`setups/medicinehistorycategories/${category.id}`).pipe(first()).subscribe(
        res => {
          if (res) {
  ...
  deleteItem(item) {
    if (item) {
      this.setup.deleteSetup(`setups/medicinehistories/${item.id}`).pipe(first()).subscribe(
        res => {
          if (res) {
  ...
  update() {
    if (this.updateForm.valid) {
      ...
      this.setup.updateSetup(this.updateForm.value, `setups/medicinehistorycategories/${this.categoryId}`)
        .pipe(first()).subscribe(
          res => {
