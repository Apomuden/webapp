...
export class ClinicServiceSetupComponent implements OnInit {
    ...
    this.setup
      ...
      .subscribe(
        data => {
          ...
          console.log(this.data, 'this is it');
        },
        error => {
          console.log(error);
          this.initLoading = false;
      ...
      this.setup
        ...
        .subscribe(
          ...
          error => {
            this.isCreatingClinicService.next(false);
            console.log(error);
            this.notification.blank("Error", `Could not create Clinic`);
          }
        );
    }
  }
    ...
    if (!this.updateForm.valid) {
      ...
      this.setup
        .updateSetup(
          ...
          `setups/clinicservices/${this.clinicServiceId}`
        )
        .pipe(first())
        .subscribe(
          response => {
  ...
  toggleItem($event: any, clinic: any) {
    this.setup
      .toggleActive(
        `setups/clinicservices/${clinic.id}`,
  ...
  deleteClinicService(clinicService: any) {
    this.setup
      .deleteSetup(`setups/clinicservices/${clinicService.id}`)
      .pipe(first())
      .subscribe(
        res => {
          this.getClinics();
