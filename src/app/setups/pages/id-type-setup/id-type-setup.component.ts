...
export class IdTypeSetupComponent implements OnInit {
  ...
  submitForm(): void {
    if (this.idType == null || this.idType === "") {
      ...
      this.setup
        .createIdType(this.idType, this.expires)
        .pipe(first())
        .subscribe(
          success => {
  ...
  toggleItem($event: any, type: any) {
    this.setup
      .toggleActive(`setups/idtypes/${type.id}`, $event ? "ACTIVE" : "INACTIVE")
      .pipe(first())
      .subscribe(
        toggled => {
  ...
  deleteIdType(idtype: any) {
    this.setup
      .deleteSetup(`setups/idtypes/${idtype.id}`)
      .pipe(first())
      .subscribe(
        res => {
          this.getIdTypes();
